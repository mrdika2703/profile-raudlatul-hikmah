...
export default function ModalCreate({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      showSuccess(response.data.message || "Rapor berhasil disimpan!");
      onClose();
      fetchRapors();
      // Reset form
      setFormData({
      ...
      const msg =
        err.response?.data?.message || "Gagal menyimpan data rapor.";
      showError(msg);
    } finally {
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-pink-100 relative animate-scale-up">
        ...
        <p className="text-xs text-slate-500 mb-6">
          Catatan kegiatan akan ditambahkan ke seluruh siswa pada kelas yang
          dipilih.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1.5">
              {formData.target === "semua"
                ? "Kegiatan akan dicatat untuk seluruh siswa di kelas ini."
                : "Kegiatan hanya dicatat untuk siswa yang hadir pada tanggal ini."}
            </p>
          </div>
          ...
          <div>
            ...
            <textarea
              required
              rows={4}
              placeholder="Contoh: Anak sangat aktif saat sesi tanya jawab dan mampu menyelesaikan tugas mewarnai dengan rapi."
